...
export default function Service() {
  ...
  const totalBill = labourCost + spareCost + otherTotal;
  ...
  const handleSpareSelect = (idx, spareName) => {
    ...
    setSpareRows(spareRows.map((r, i) => i === idx ? { ...r, spareId: found?._id || '', spareName, sellingPrice: found?.sellingPrice || '' } : r));
  };
  const handleSpareChange = (idx, field, val) => setSpareRows(spareRows.map((r, i) => i === idx ? { ...r, [field]: val } : r));
  const addSpareRow = () => setSpareRows([...spareRows, emptySpare()]);
  const removeSpareRow = (idx) => spareRows.length === 1 ? setSpareRows([emptySpare()]) : setSpareRows(spareRows.filter((_, i) => i !== idx));

  // Other charges handlers
  const handleChargeChange = (idx, field, val) => setOtherCharges(otherCharges.map((c, i) => i === idx ? { ...c, [field]: val } : c));
  const addCharge = () => setOtherCharges([...otherCharges, emptyCharge()]);
  const removeCharge = (idx) => otherCharges.length === 1 ? setOtherCharges([emptyCharge()]) : setOtherCharges(otherCharges.filter((_, i) => i !== idx));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await addService({
        ...
        spareItems: validSpares.map(r => ({ spareId: r.spareId || undefined, spareName: r.spareName.trim(), sellingPrice: parseFloat(r.sellingPrice) || 0, quantity: parseFloat(r.quantity) || 1 })),
      ...
      setForm(emptyForm); setSpareRows([emptySpare()]); setOtherCharges([emptyCharge()]);
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm">Billing · Multi-payment tracking · Spare stock auto-update</p>
        </div>
        <button onClick={() => { setShowForm(!showForm); setForm(emptyForm); setSpareRows([emptySpare()]); setOtherCharges([emptyCharge()]); }} className="btn-primary">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="label">Service Type <span className="text-slate-500 text-xs font-normal">(describe manually)</span></label>
                  <input name="serviceType" value={form.serviceType} onChange={handleChange} placeholder="e.g. Full service + brake check" className="input-field" />
                </div>
                <div>
                    ...
                    return (
                      <div key={idx} className="grid grid-cols-12 gap-2 items-center">
                        <div className="col-span-5">
                          <select value={row.spareName} onChange={(e) => handleSpareSelect(idx, e.target.value)} className="input-field text-sm py-2">
                            <option value="">-- Select Part --</option>
                            {sparesList.map(s => <option key={s._id} value={s.spareName}>{s.spareName} (Stock: {s.quantity})</option>)}
                            <option value="__manual__">+ Type manually</option>
                          </select>
                          {row.spareName === '__manual__' && (
                            <input className="input-field text-sm py-2 mt-1" placeholder="Enter part name" onChange={(e) => handleSpareChange(idx, 'spareName', e.target.value)} />
                          )}
                        </div>
                        <div className="col-span-3">
                          <input type="number" value={row.sellingPrice} onChange={(e) => handleSpareChange(idx, 'sellingPrice', e.target.value)} placeholder="Auto-filled" className="input-field text-sm py-2" />
                        </div>
                        <div className="col-span-2">
                          <input type="number" min="1" value={row.quantity} onChange={(e) => handleSpareChange(idx, 'quantity', e.target.value)} className="input-field text-sm py-2" />
                        </div>
                        <div className="col-span-1 text-emerald-400 text-sm font-semibold text-right">{fmt(rowTotal)}</div>
                        <div className="col-span-1 flex justify-end">
                          <button type="button" onClick={() => removeSpareRow(idx)} className="w-7 h-7 flex items-center justify-center rounded bg-red-900/40 hover:bg-red-800 text-red-400 text-xs">✕</button>
                        </div>
                      </div>
                    );
                  })}
                </div>
                <button type="button" onClick={addSpareRow} className="mt-3 text-emerald-400 text-sm hover:text-emerald-300 flex items-center gap-1">
            ...
            <div>
              <p className="text-slate-400 text-xs font-semibold uppercase tracking-wider mb-3">④ Other Charges <span className="text-slate-500 normal-case font-normal">(Courier, Transport, Packaging, etc.)</span></p>
              <div className="border border-slate-600 rounded-xl p-4 bg-slate-700/20">
                <div className="grid grid-cols-12 gap-2 mb-2 px-1">
                  ...
                  {otherCharges.map((c, idx) => (
                    <div key={idx} className="grid grid-cols-12 gap-2 items-center">
                      <div className="col-span-8">
                        <input value={c.description} onChange={(e) => handleChargeChange(idx, 'description', e.target.value)} placeholder="e.g. Courier, Transport, Packaging" className="input-field text-sm py-2" />
                      </div>
                      <div className="col-span-3">
                        <input type="number" value={c.amount} onChange={(e) => handleChargeChange(idx, 'amount', e.target.value)} placeholder="0" className="input-field text-sm py-2" />
                      </div>
                      <div className="col-span-1 flex justify-end">
                        <button type="button" onClick={() => removeCharge(idx)} className="w-7 h-7 flex items-center justify-center rounded bg-red-900/40 hover:bg-red-800 text-red-400 text-xs">✕</button>
                      </div>
                    </div>
                  ))}
                </div>
                <button type="button" onClick={addCharge} className="mt-3 text-emerald-400 text-sm hover:text-emerald-300 flex items-center gap-1">
            ...
            <div className="bg-slate-700/50 border border-slate-600 rounded-xl p-5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                ...
                <div className="bg-emerald-900/40 border border-emerald-700 rounded-lg p-3"><p className="text-emerald-400 text-xs mb-1 font-semibold">Total Bill</p><p className="text-emerald-400 font-bold text-xl">{fmt(totalBill)}</p></div>
              </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="table-header">
                  {['Customer', 'Vehicle', 'Service Type', 'Spares', 'Labour', 'Others', 'Total', 'Paid', 'Pending', 'Status', 'Date', 'Completed On', 'Actions'].map(h => (
                ...
                {services.map((s) => (
                  <React.Fragment key={s._id}>
                    <tr className="table-row cursor-pointer" onClick={() => setExpandedRow(expandedRow === s._id ? null : s._id)}>
                      ...
                      <td className="px-4 py-3 text-slate-300 max-w-[130px] truncate" title={s.serviceType}>{s.serviceType || '—'}</td>
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6">
              ...
              <div className="bg-slate-700/30 rounded-lg p-3 mb-5 flex flex-wrap gap-4 text-sm">
                <div><span className="text-slate-400">Service Date: </span><span className="text-white">{fmtDate(payModal.serviceDate)}</span></div>
                <div><span className="text-slate-400">Status: </span>{statusBadge(payModal.paymentStatus)}</div>
                {payModal.paymentCompletionDate && (
                  <div><span className="text-slate-400">Completed On: </span><span className="text-emerald-400 font-semibold">{fmtDate(payModal.paymentCompletionDate)}</span></div>
                )}
              </div>
                ...
                <div className="bg-slate-700/30 border border-slate-600 rounded-xl p-4 mb-5">
                  ...
                  <form onSubmit={handleAddPayment} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input type="date" value={payForm.paymentDate} onChange={(e) => setPayForm({ ...payForm, paymentDate: e.target.value })} className="input-field" />
                    </div>
                    <div>
                      <label className="label">Note (optional)</label>
                      <input value={payForm.note} onChange={(e) => setPayForm({ ...payForm, note: e.target.value })} placeholder="e.g. 2nd installment" className="input-field" />
                    </div>
